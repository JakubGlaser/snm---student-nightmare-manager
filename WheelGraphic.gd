...
func _draw_label(center: Vector2, radius: float, angle: float, text: String) -> void:
	...
	var ts := label_font.get_string_size(text, HORIZONTAL_ALIGNMENT_CENTER, band, fs)
	var x : float = -outer if flip else inner
	var y : float = ts.y * 0.30
	draw_string_outline(label_font, Vector2(x, y), text, HORIZONTAL_ALIGNMENT_CENTER, band, fs, 5, CREAM)
	draw_string(label_font, Vector2(x, y), text, HORIZONTAL_ALIGNMENT_CENTER, band, fs, TEXT_COLOR)
